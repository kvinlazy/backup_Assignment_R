...
shinyServer(function(input, output) {
   ...
   colin1 <- reactive({
     weatherAUS[,c(input$Colfactor1,input$Colfactor2)]
   })
   colin2 <- reactive({
     weatherAUS[,c(input$Colfactor2)]
   })
    
    output$text <- renderPlot({
      test <- weatherAUS[,c(input$Colfactor1,input$Colfactor2)]
      s <- names(test)
    ...
    output$distPlot1 <- renderLeaflet({
        ...
        leaflet() %>% 
          ...
          addMarkers(map, lat = locationsAUS$latitude,
